...
import { commitDaybook } from "@/features/recovery/service.server";

/** Commit a Day Book upload: reset + set SR/CR and Live Recovery for the month's Recovery Plans. */
export async function POST(req: NextRequest) {
  return handle(async () => {
    ...
    return ok(await commitDaybook(auth, await fileToBuffer(file), file.name, data));
  });
}
