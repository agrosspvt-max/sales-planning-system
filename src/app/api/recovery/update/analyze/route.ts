...
import { analyzeRecoveryUpdate } from "@/features/recovery/service.server";

/** Preview an Update Recovery: which active plans for the month refresh + aggregate change. */
export async function POST(req: NextRequest) {
  return handle(async () => {
    ...
    return ok(await analyzeRecoveryUpdate(auth, await fileToBuffer(file), file.name, data));
  });
}
