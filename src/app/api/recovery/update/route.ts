...
import { updateRecoveryFromAging } from "@/features/recovery/service.server";

/** Update Recovery: refresh aging/business data of every active recovery plan for a month. */
export async function POST(req: NextRequest) {
  return handle(async () => {
    ...
    return ok(await updateRecoveryFromAging(auth, await fileToBuffer(file), file.name, data));
  });
}
